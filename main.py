...
_INSTANCE_NAME = 'jcmarks-mobile:us-central1:mobile-data'
...
_EPSILON = 1
# the table with device usage
_USAGE = 'plugin_device_usage'
...
def index():
    ...
    if (os.getenv('SERVER_SOFTWARE') and
        ...
        ##Double hash = Jordan is trying to comment out the code to make it run.
        ...
        ##Edited for the table I do have
        day = "FROM_UNIXTIME(timestamp/1000,'%Y-%m-%d')"
        query = "SELECT {0} as day_with_data, count(*) as records FROM {1} GROUP by day_with_data".format(day, _USAGE)
